...
def sup_fit(x,y,ams):
    ...
    xc,yc,_ = origins(xax,yax,ams)
    params = Parameters()
    params.add('a',value=1,min=1e-6)
    for i in range(ams+1):
        params.add('p'+str(i),value=1)
        params.add('m'+str(i),value=-0.1)
    ...
    outdict['xc']=xc;outdict['yc']=yc;outdict['offset']=offset
    return y-fn_sum(outdict,x,y),outdict
